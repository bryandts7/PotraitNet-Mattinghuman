...
def show_edge(mask_ori):
    ...
    countours, _ = cv2.findContours(binary, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) # RETR_EXTERNAL
    '''
    ...
    return diff   
    '''
    cv2.drawContours(myImg, countours, -1, (255, 255, 255), 4)
    return myImg
...
class PortraitSeg(data.Dataset): 
    ...
    def __getitem__(self, index):
        ...
        if self.dataset == "matting_human":
            ...
            # load mask
            annopath = os.path.join(self.AnnoRoot, img_id.replace('.jpg', '.png').replace('clip', 'matting'))
            matting = cv2.imread(annopath)
        ...
        if self.dataset == "easypotrait":
            # basic info
            img_id = self.imgIds[index].strip()
            img_path = os.path.join(self.ImageRoot, img_id.replace('.png', '.jpg'))
            img = cv2.imread(img_path)
            img_name = img_path[img_path.rfind('/')+1:]
            
            # load mask
            annopath = os.path.join(self.AnnoRoot, img_id)
            mask = cv2.imread(annopath, 0)
            # mask = cv2.imread(annopath.replace('Labels', 'LabelsAug'), 0)
...
class Human(data.Dataset): 
    def __init__(self, exp_args):
        ...
        if 'matting_human' in self.datasetlist:
            ...
            exp_args.dataset = 'matting_human'
            self.datasets['matting_human'] = PortraitSeg(ImageRoot, AnnoRoot, ImgIds_Train, ImgIds_Test, self.exp_args)
        
        if 'easypotrait' in self.datasetlist:
            ImageRoot = self.data_root + 'images/'
            AnnoRoot = self.data_root + 'annotations/'
            ImgIds_Train = self.file_root + 'easypotrait_train.txt'
            ImgIds_Test = self.file_root + 'easypotrait_val.txt'
            exp_args.dataset = 'easypotrait'
            self.datasets['easypotrait'] = PortraitSeg(ImageRoot, AnnoRoot, ImgIds_Train, ImgIds_Test, self.exp_args)
